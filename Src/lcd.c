...
// V4.1 20211208
// 修改NT5510
// ID读取方式,改为先发送秘钥,然后读取C500和C501,从而获取正确的ID(0X5510) V4.2
// 20211222 1，修改FSMC速度，兼容7789显示OV2640数据 2，解决因NT5510
// ID读取（发送C501指令）导致SSD1963误触发软件复位进而读取不到ID问题，加延时解决
//////////////////////////////////////////////////////////////////////////////////
...
uint32_t LCD_ReadPoint(uint16_t x, uint16_t y) {
    ...
    opt_delay(2);
    b = LCD_RD_DATA();
    g = r & 0XFF;  // 对于 9341/5310/5510/7789,
                   // 第一次读取的是RG的值,R在前,G在后,各占8位
    g <<= 8;
    return (((r >> 11) << 11) | ((g >> 10) << 5) |
            (b >> 11));  // 9341/5310/5510/7789 需要公式转换一下
}
...
void LCD_Init(void) {
    ...
    TFTSRAM_Handler.Init.DataAddressMux =
        FSMC_DATA_ADDRESS_MUX_DISABLE;  // 地址/数据线不复用
    ...
    TFTSRAM_Handler.Init.ExtendedMode =
        FSMC_EXTENDED_MODE_ENABLE;  // 读写使用不同的时序
    ...
    FSMC_ReadWriteTim.AddressSetupTime =
        0x0F;  // 地址建立时间（ADDSET）为16个HCLK 1/168M=6ns*16=96ns
    FSMC_ReadWriteTim.AddressHoldTime = 0;
    FSMC_ReadWriteTim.DataSetupTime =
        60;  // 数据保存时间为60个HCLK	=6*60=360ns
    FSMC_ReadWriteTim.AccessMode = FSMC_ACCESS_MODE_A;  // 模式A
    // FMC写时序控制寄存器
    FSMC_WriteTim.BusTurnAroundDuration =
        0;  // 总线周转阶段持续时间为0，此变量不赋值的话会莫名其妙的自动修改为4。导致程序运行正常
    FSMC_WriteTim.AddressSetupTime = 9;  // 地址建立时间（ADDSET）为9个HCLK
                                         // =54ns
        ...
        lcddev.id == 0X1963)  // 如果是这几个IC,则设置WR时序为最快
    {
        ...
        {
            FSMC_Bank1E->BWTR[6] |=
                3 << 8;  // 数据保存时间(DATAST)为6ns*4个HCLK=24ns
        } else {
            FSMC_Bank1E->BWTR[6] |=
                2 << 8;  // 数据保存时间(DATAST)为6ns*3个HCLK=18ns
        }
    }
